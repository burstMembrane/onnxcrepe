...
def bins_to_cents(bins, apply_dither=False):
    ...
    cents = crepetrt.CENTS_PER_BIN * bins + 1997.3794084376191
...
def cents_to_bins(cents, quantize_fn=np.floor):
    ...
    bins = (cents - 1997.3794084376191) / crepetrt.CENTS_PER_BIN
...
def dither(cents):
    ...
    noise = scipy.stats.triang.rvs(c=0.5,
                                   loc=-crepetrt.CENTS_PER_BIN,
                                   scale=2 * crepetrt.CENTS_PER_BIN,
                                   size=cents.shape)
